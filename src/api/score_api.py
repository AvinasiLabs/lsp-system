...
async def get_valid_scores(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="获取有效积分失败")
...
async def get_score_history(
    ...
    try:
        ...
        history = persistence_service.get_user_score_history(
            user_id, start, end, include_expired
        )
        ...
        raise HTTPException(status_code=500, detail="获取积分历史失败")
...
async def get_expiring_scores(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="获取即将过期积分失败")
...
async def get_tier_statistics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="获取等级统计失败")
...
async def check_and_mark_expired():
    ...
    try:
        ...
        return ExpirationCheckResponse(
            ...
            message=f"检查了{checked}条记录，标记了{expired}条为过期"
        )
    except Exception as e:
        logger.error(f"检查过期积分失败: {e}")
        raise HTTPException(status_code=500, detail="检查过期积分失败")
...
async def get_monthly_summary(
    ...
    try:
        ...
        # 获取历史记录
        history = persistence_service.get_user_score_history(
            user_id, start_date, end_date, include_expired=False
        )
