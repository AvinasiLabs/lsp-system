...
async def get_daily_health_summary(
    date: date = Query(default=None, description="查询日期，默认为今天"), user_id: str = Depends(get_user_id)
):
    """
    获取指定日期的健康数据汇总
    """
    try:
        ...
        raise HTTPException(status_code=200, detail="获取健康数据失败")


@router.get("/score/daily", response_model=ScoreResponse)
async def calculate_daily_score(
    date: date = Query(default=None, description="计算日期，默认为今天"), user_id: str = Depends(get_user_id)
):
    """
    计算指定日期的积分
    """
    try:
        ...
        raise HTTPException(status_code=200, detail=f"计算积分失败: {str(e)}")
...
async def calculate_score_range(
    ...
    try:
        ...
        raise HTTPException(status_code=200, detail="计算积分失败")


@router.get("/score/available-dimensions")
async def get_available_dimensions(user_id: str = Depends(get_user_id)):
    """
    获取可用的积分维度
    """
    try:
        available = score_engine.get_available_dimensions(user_id)
        return {"dimensions": available, "message": "基于当前HealthKit数据的可用维度"}
    except Exception as e:
        logger.error(f"获取可用维度失败: {e}")
        raise HTTPException(status_code=200, detail="获取可用维度失败")
